...
export default function GetQuoteModal({
  ...
  // BUG FIX: Removed activePanel.code from dependencies so background scroll doesn't reset selection
  useEffect(() => {
    if (isOpen) {
      // Only run this ONCE when the modal opens. 
      // If the background auto-scrolls and changes activePanel, we DO NOT want to add it to selectedCodes.
  ...
  const defaultAutoMessage = useMemo(() => {
    ...
    const qtyText = quantityMode === 'direct' ? `approx ${approxSqFt} sq ft (${quantity} panels)` : `quantity to be discussed`;
  ...
  }, [selectedPanels, activePanel.name, quantity, approxSqFt, quantityMode]);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      {/* Modal Container */}
      <div 
        className="relative z-10 w-full max-w-5xl rounded-2xl md:rounded-[2rem] bg-white dark:bg-[#111111] shadow-2xl flex flex-col md:flex-row max-h-[90vh] md:max-h-[85vh] overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-stone-200 dark:border-white/10"
      ...
      >
        ...
        {/* Right Side: Static-style Form Content */}
        <div className="flex-1 flex flex-col min-h-0 bg-stone-50 dark:bg-[#161616]">
          {!isSubmitted ? (
            <>
              ...
              <div className="flex-shrink-0 flex items-center justify-between px-6 py-3.5 border-b border-stone-200 dark:border-white/5 bg-white dark:bg-[#111111]">
                <div>
                  ...
                  <p className="text-xs text-stone-500 mt-1 md:hidden">
                    {seriesLabel}
                  </p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                ...
                {/* 1. Panel Selection (Wrap, no auto scroll) */}
                <section>
                  ...
                  {/* Flex wrap instead of horizontal scrolling */}
                ...
                <section>
                  ...
                  {/* Inline flex for pill style rather than full width block */}
            ...
            /* SUCCESS STATE */
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500 bg-white dark:bg-[#111111]">
              <div className="relative mb-6">
                <div className="absolute inset-0 rounded-full bg-amber-500/20 animate-ping" />
                <div className="relative flex w-20 h-20 items-center justify-center rounded-full bg-gradient-to-tr from-amber-500 to-amber-400 text-white shadow-xl shadow-amber-500/30">
              ...
              <p className="text-sm text-stone-600 dark:text-stone-400 mb-8 max-w-sm">
                WhatsApp has been opened with your requirements. Our sales team will get back to you with the wholesale pricing.
              </p>

              <div className="w-full max-w-sm bg-stone-100 dark:bg-white/5 rounded-xl p-4 mb-8 border border-stone-200 dark:border-white/10">
                <p className="text-xs uppercase tracking-wider text-stone-500 dark:text-stone-400 font-bold mb-1">
                  Reference ID
                </p>
                <p className="font-mono text-lg font-bold text-amber-600 dark:text-amber-500">
                  {rfqRefId}
                </p>
              </div>
              ...
              <div className="flex gap-3 w-full max-w-sm">
                <a
                  ...
                  className="flex-1 rounded-xl bg-amber-500 hover:bg-amber-600 px-4 py-3 text-sm font-bold text-white transition-colors flex items-center justify-center"
                >
                  Open WhatsApp
                </a>
                <button
      ...
      {/* Required for custom scrollbar styling if not already global */}
      <style jsx global>{`
